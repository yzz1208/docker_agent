...
AgentRoute = Literal["chat", "docs_only", "runtime_tools", "clarify"]
...
def parse_route_decision(raw: str) -> AgentRouteDecision:
    ...
    if route_raw not in {"chat", "docs_only", "runtime_tools", "clarify"}:
    ...
    if route == "chat":
        if tools:
            raise AgentRoutingError("chat route must not contain runtime tools")
        if container_ref is not None:
            raise AgentRoutingError("chat route must not contain container_ref")
        clarification = None
        use_docs = False
